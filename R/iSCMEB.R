...
RunHarmonyLouvain <- function(PRECASTObj, resolution=0.5){
  ...
  set.seed(seed)
  tic <- proc.time()
  hZ_harmony_profastP <- HarmonyMatrix(matlist2mat(PRECASTObj@resList$FAST$hV), meta_data = data.frame(sample = sampleID),
  ...
  lm1 <- lm(matlist2mat(PRECASTObj@resList$FAST$hV)~hZ_harmony_profastP)
