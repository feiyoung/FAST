...
We demonstrate the use of FAST to three simulated ST data that are [here](https://github.com/feiyoung/FAST/tree/main/vignettes_data), which can be downloaded to the current working path by the following command:
```{r eval=FALSE}
githubURL <- "https://github.com/feiyoung/FAST/blob/main/vignettes_data/simu3.rds?raw=true"
...
```{r  eval = TRUE}
seuInt <- AddUMAP(seuInt, n_comp=3, reduction = 'harmony', assay = 'FAST')
seuInt
```
...
```{r  eval = TRUE, fig.width=9, fig.height=3.3}
seuInt <- AddTSNE(seuInt, n_comp = 2, reduction = 'harmony', assay = 'FAST')
